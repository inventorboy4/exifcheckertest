...
def image_sort(path, res):
    ...
    for address, dirs, files in os.walk(path):
        for filename in files:
            amounts[0] += 1
            if filename.lower().split(".")[-1] not in valid_extentions:
...
#            print(datetime.fromtimestamp(os.path.getctime(full_path)).strftime('%Y-%m-%d %H:%M:%S'))
            ...
            if ImageDate(full_path) != None:
                ...
                res_path = res+"/"+new_filename[:7]
                if not os.path.exists(res_path):
                    os.makedirs(res_path)
                if not os.path.exists(res_path+"/"+new_filename):
                    shutil.copy2(full_path, res_path+"/"+new_filename)
        #for dir in dirs:
...
output = args.output if (args.output != None) else os.getcwd()+"/sorted"
print(path)
amounts = [0, 0]
for folder in path:
